...
namespace CTRPluginFramework
{
    ...
    void WatchPointManager::DisableWatchPoint(u32 address)
    {
        auto& manager = _singleton;
        ...
        for (u32 id = 0; id < 2; ++id)
        {
            if (manager._watchPoints[id]._address == address)
            {
                svcKernelSetState(id == 0 ? 0x10004 : 0x10005, id);

                WatchPoint& watchpoint = manager._watchPoints[id];
                watchpoint._address = 0;
                watchpoint._size = 0;
                watchpoint._kind = WatchPoint::Disabled;
                watchpoint._totalHit = 0;
